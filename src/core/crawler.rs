...
mod tests {
    ...
    async fn test_crawler() {
        ...
        let repo = Arc::new(Repository::new(config.clone()).await.unwrap());
        tokio::spawn(async move {
            let r = dispatch.run(repo.clone()).await;
            println!("{:#?}", r);
        });

        let task = Task {
            images: vec!["http://123.0.0.1".to_string()],
            tags: vec![],
            literatures: vec![],
            headers: Default::default(),
            source_site: "all".to_string(),
            extra: None,
        };

        let (id_tx, id_rx) = tokio::sync::oneshot::channel();

        let inner = InnerTask { task, id_tx };

        tx.send(inner).await.unwrap();
