...
pub async fn format(bytes: &[u8], base_path: &PathBuf, tid: i32, pid: i32) -> Result<PathBuf, Error> {
    let image = image::load_from_memory(bytes)?.to_rgba8();
    let encoder = webp::Encoder::from_rgba(&image, image.width(), image.height());
    let mem = encoder.encode(100.0);

    let target = base_path.join(format!("{:0>10}", tid));

    tokio::fs::create_dir_all(&target).await?;

    let target = target.join(format!("{:0>10}.webp", pid));

    tokio::fs::write(&target, mem.as_bytes()).await?;
...
mod tests {
    ...
    async fn test_format() {
        ...
        format(&bytes, &base_path, 1, 123).await.unwrap();
    }
